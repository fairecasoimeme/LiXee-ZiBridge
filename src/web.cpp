...
const char HTTP_HEADER[] PROGMEM = 
  ...
  " </head>"
    "<body>"
  "<nav class='navbar navbar-expand-lg navbar-light bg-light rounded'><a class='navbar-brand' href='/'><img src='web/img/logo.png'/> <strong>ZiBridge </strong>"
   VERSION
  "</a>"
  ...
  "<ul class='navbar-nav'>"
    ...
    "<li class='nav-item dropdown'>"
      ...
      "<div class='dropdown-menu' aria-labelledby='navbarDropdown'>"         
          ...
          "<a class='dropdown-item' href='/serial'>Radio type</a>"      
...
const char HTTP_SERIAL[] PROGMEM = 
 "<h1>Config Radio device</h1>"
  "<div class='row justify-content-md-center' >"
    "<div class='col-sm-6'><form method='POST' action='saveSerial'>"
  "<div class='form-group'>"
   "<label for='baud'>Radio type</label>"
   "<select class='form-control' id='radio' name='radio_mdns'>"
      "<option value='zigate' {{selectedzigate}}>ZiGate</option>"
      "<option value='znp' {{selectedznp}}>ZNP</option>"
      "<option value='efr32' {{selectedezsp}}>EZSP</option>"
      "<option value='conbee' {{selectedconbee}}>Conbee</option>"
    "</select>"
  "</div>" 
   "<div class='form-group'>"
  ...
  "</div>"
  "<div class='form-group'>"
    "<label for='dataflow'>Flow control</label>"
    "<select class='form-control' id='dataflow' name='dataflow_mdns'>"
      "<option value='software' {{selectedsoft}}>Software</option>"
      "<option value='hardware' {{selectedhard}}>Hardware</option>"
    "</select>"
  "</div>"
   "<br><br>"
  "<button type='submit' class='btn btn-primary mb-2'name='save'>Save</button>"
...
const char HTTP_HELP[] PROGMEM = 
 "<h1>Help !</h1>"
    ...
    "Please go here :</br>"
    "<a href=\"https://github.com/fairecasoimeme/LiXee-ZiBridge\" target='_blank'>Sources</a>";
...
"<div class='row justify-content-md-center' >"
    ...
    "<input class='form-check-input' id='modeUSB' type='checkbox' name='modeUSB' {{modeUSB}}>"
    "<label class='form-check-label' for='modeUSB'>Disable gateway. Mode USB only (switch button on FTDI)</label>"
    "<br>"
  "</div>"
  "<button type='submit' class='btn btn-primary mb-2'name='save'>Save</button>"
  "</form></div>"
...
void handleGeneral(AsyncWebServerRequest * request) {
  ...
  if (ConfigSettings.modeUSB)
  {
    result.replace("{{modeUSB}}","checked");
  }else{
    result.replace("{{modeUSB}}","");
  }
...
void handleSerial(AsyncWebServerRequest * request) {
  ...
  if (memcmp(ConfigSettings.radioType,"zigate",6)==0)
  {
     result.replace("{{selectedzigate}}","Selected");
  }else if (memcmp(ConfigSettings.radioType,"efr32",4)==0){
     result.replace("{{selectedezsp}}","Selected");
  }else if (memcmp(ConfigSettings.radioType,"znp",3)==0){
     result.replace("{{selectedznp}}","Selected");
  }else if (memcmp(ConfigSettings.radioType,"conbee",6)==0){
     result.replace("{{selectedconbee}}","Selected");
  }
  ...
  if (memcmp(ConfigSettings.dataFlow,"hardware",8)==0)
  {
     result.replace("{{selectedhard}}","Selected");
  }else{
     result.replace("{{selectedesoft}}","Selected");
  }
...
void handleSaveGeneral(AsyncWebServerRequest * request)
{
  ...
  if (request->arg("refreshLogs").toDouble()<1000)
  {
   ...
   StringConfig = "{\"disableWeb\":"+disableWeb+",\"modeUSB\":"+modeUSB+",\"enableHeartBeat\":"+enableHeartBeat+",\"refreshLogs\":"+refreshLogs+"}";    
...
void handleSaveSerial(AsyncWebServerRequest * request)
{
  ...
  String radioType= request->arg("radio_mdns");
  String dataFlow= request->arg("dataflow_mdns");
  const char * path="/config/configSerial.json";

  StringConfig = "{\"baud\":"+serialSpeed+",\"radioType\":\""+radioType+"\",\"dataFlow\":\""+dataFlow+"\"}";
