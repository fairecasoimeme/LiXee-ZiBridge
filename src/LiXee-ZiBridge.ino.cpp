...
#define DEVICE_NAME "LiXee-ZiBridge"
...
bool loadConfigGeneral() {
  ...
  ConfigSettings.modeUSB = (int)doc["modeUSB"];
...
bool loadConfigSerial() {
  ...
  strlcpy(ConfigSettings.radioType, doc["radioType"] | "", sizeof(ConfigSettings.radioType));
  strlcpy(ConfigSettings.dataFlow, doc["dataFlow"] | "", sizeof(ConfigSettings.dataFlow));
  configFile.close();
  return true;
}
...
*/ 
  ...
  //Zeroconf
  String localdns = "LiXee-ZiBridge";
  ...
  MDNS.addService("zigate-zigbee-gateway", "tcp", ConfigSettings.tcpListenPort);
  MDNS.addServiceTxt("zigate-zigbee-gateway", "tcp", "version", "1.0");
  MDNS.addServiceTxt("zigate-zigbee-gateway", "tcp", "radio_type", (String)ConfigSettings.radioType);
  MDNS.addServiceTxt("zigate-zigbee-gateway", "tcp", "baud_rate", (String)ConfigSettings.serialSpeed);
  MDNS.addServiceTxt("zigate-zigbee-gateway", "tcp", "data_flow_control", (String)ConfigSettings.dataFlow);
...
void loop(void)
{
 ...
 if (!ConfigSettings.modeUSB)
 {
  ...
  if (loopCount > 200000)
  {
    MDNS.queryService("zigate-zigbee-gateway", "tcp");
